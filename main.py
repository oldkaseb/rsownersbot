...
BOT_USERNAME: str = ""
db = None  # ← این خط رو همین‌جا بذار
# -------------------- Texts --------------------
WELCOME_TEXT = """سلام! 👋
به ربات ارتباط سریع با پشتیبانی راینوسول خوش آمدید مراحل ارتباط در منوی زیر:"""
MAIN_MENU_TEXT = "یکی از گزینه‌ها را انتخاب کنید:"

# Buttons
BTN_SECTION_BOTS   = "🤖درخواست پشتیبانی سریع"
BTN_SECTION_SOULS  = "💬 گروه Souls"
BTN_SECTION_VSERV  = "🛍️خدمات تیم راینو"
BTN_SECTION_FREE   = "🗣️آزاد"
...
async def cmd_start(m: Message):
    media_row = await db.fetchrow("SELECT * FROM start_media LIMIT 1")
    if media_row:
        ...
        try:
            if media_type == "photo":
                await m.answer_photo(photo=file_id, caption=caption, reply_markup=main_menu_kb())
            elif media_type == "video":
                await m.answer_video(video=file_id, caption=caption, reply_markup=main_menu_kb())
            elif media_type == "animation":
                await m.answer_animation(animation=file_id, caption=caption, reply_markup=main_menu_kb())
            elif media_type == "document":
                await m.answer_document(document=file_id, caption=caption, reply_markup=main_menu_kb())
            elif media_type == "sticker":
                await m.answer_sticker(sticker=file_id)  # استیکر کپشن و دکمه نمی‌گیره
                await m.answer("👇 لطفاً یکی از گزینه‌ها را انتخاب کنید:", reply_markup=main_menu_kb())
        except Exception as e:
            await m.answer(f"❌ خطا در ارسال رسانه: {e}")
    else:
        await m.answer("👋 خوش آمدید!", reply_markup=main_menu_kb())
...
async def cmd_set_start_media(m: Message):
    ...
    await db.execute(
        "INSERT INTO start_media (file_id, media_type, caption) VALUES ($1, $2, $3)",
    ...
    if contains_malek(text):
        ...
        if BOT_USERNAME:
            btns = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(
                    text="RHINOSOUL پیام به پشتیبانی",
        ...
        # ⬇️ پیام ربات
        sent = await m.reply(
            " @RHINOSOUL_TM سلام.من منشی تیم راینو سول هستم میتونی داخل پیوی من با پشتیبانی راینو ارتباط بگیری (صفر تا صد هر سرویس)",
            reply_markup=btns
        )
        # ⬇️ حذف خودکار همون پیام بعد از ۳۰ ثانیه
        asyncio.create_task(_auto_delete(sent.chat.id, sent.message_id, delay=50))
